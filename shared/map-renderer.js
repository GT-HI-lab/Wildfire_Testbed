import { AGENT_TYPES, SECTION_KEYS, findBlockingTree, terrainAt } from "./simulation.js";
...
export function renderMap(canvas, state, options = {}) {
  ...
  const clearedCells = new Set(state.clearedTerrain || []);
  ...
  for (let x = 0; x < size; x += stride) {
    for (let y = 0; y < size; y += stride) {
      const terrainColor = clearedCells.has(`${Math.round(x)},${Math.round(y)}`)
        ? COLORS.grass
        : COLORS[terrainAt(x, y)];
      ctx.fillStyle = visibleCells && !blockVisible(visibleCells, x, y, stride) ? "#030604" : terrainColor;
...
function collectScenery(state, observer) {
  ...
  for (let x = Math.floor(observer.x - 55); x <= observer.x + 55; x += 5) {
    for (let y = Math.floor(observer.y - 55); y <= observer.y + 55; y += 5) {
      ...
      if (clearedCells.has(`${Math.round(x)},${Math.round(y)}`)) continue;
  ...
  const lookAhead = {
    x: observer.x + Math.cos(observer.heading || 0) * 18,
    y: observer.y + Math.sin(observer.heading || 0) * 18
  };
  const blockingTree = observer.blockedTree || findBlockingTree(state, observer, lookAhead);
  if (blockingTree && !clearedCells.has(blockingTree.key)) {
    items.push({
      ...blockingTree,
      kind: "tree",
      variant: 0,
      distance: Math.hypot(blockingTree.x - observer.x, blockingTree.y - observer.y)
    });
  }
...
function projectToView(item, observer, rect, horizon) {
  ...
  if (forward <= 0.25 || forward > 75 || Math.abs(lateral) > forward * 1.05) return null;
