...
const DRONE_SPEED = 1.55;
const HELICOPTER_SPEED = 5;
const RELIABILITY_PHASE_SECONDS = MISSION_DURATION_SECONDS / 4;
export const COMMUNICATION_STYLES = ["transparent", "explainable", "adaptive"];
export const RELIABILITY_PHASES = [
  { label: "R1 High / High", helicopter: "high", drone: "high" },
  { label: "R2 Helicopter degraded", helicopter: "low", drone: "high" },
  { label: "R3 Drone degraded", helicopter: "high", drone: "low" },
  { label: "R4 Joint degraded", helicopter: "low", drone: "low" }
];
...
export function createInitialSession(id = "pilot-001", options = {}) {
  ...
  const state = {
    ...
    pauseReason: "",
    tick: 0,
    condition: options.testbed === "high" ? "High" : "Mixed",
    ...
    reliabilityPhase: { index: 0, ...RELIABILITY_PHASES[0] },
    task:
  ...
  pushEvent(state, "session_start", "Session initialized", {
    ...
    reliabilityPhase: state.reliabilityPhase,
...
export function ensureSessionShape(state) {
  ...
  state.version = Math.max(state.version || 0, 9);
  state.agents ||= {};
  state.metrics ||= {};
  state.events ||= [];
  state.pendingEvents ||= [];
  ...
  if (["High-High", "Low-Low"].includes(state.condition)) state.condition = "High";
  if (!COMMUNICATION_STYLES.includes(state.communicationStyle)) state.communicationStyle = "transparent";
  state.reliabilityPhase ||= { index: 0, ...RELIABILITY_PHASES[0] };
  if (previousVersion < 9) updateReliabilityPhase(state, true);
...
export function applyReliabilityPairing(state, pairing) {
  state.condition = pairing === "High" || pairing === "High-High" ? "High" : "Mixed";
  updateReliabilityPhase(state, true);
  state.coordination.helicopterAreaCheckRequested = false;
  if (state.condition === "High" && state.experiment?.malfunctionActive) {
    recoverMalfunction(state, "condition_changed");
  }
  pushEvent(
    state,
    "condition_changed",
    `Testbed set to ${state.condition}; reliability phase is ${state.reliabilityPhase.label}`,
    { condition: state.condition, reliability: state.reliability, reliabilityPhase: state.reliabilityPhase }
  );
}
...
export function parseHelicopterCommand(text, state) {
  ...
  if (coord || /(move|go|fly|scout|inspect|head).*(fire|smoke|coordinate|sector|area|zone)/.test(lower)) {
    ...
    if (!isDroneExplored(state, target)) {
      return {
        type: ACTIONS.HELI_IDLE,
        x: 0,
        y: 0,
        description: `Waiting for Drone AI to scout (${Math.round(target.x)}, ${Math.round(target.y)})`,
        needsScout: true,
  ...
  if (/(refill|reload).*(water)?/.test(lower)) {
    ...
    if (!isDroneExplored(state, water)) {
      return {
        type: ACTIONS.HELI_IDLE,
        x: 0,
        y: 0,
        description: `Waiting for Drone AI to scout water at (${Math.round(water.x)}, ${Math.round(water.y)})`,
        needsScout: true,
...
export function helicopterAgentReply(participantText, state) {
  ...
  const responseByType = {
    [ACTIONS.HELI_IDLE]: command.needsScout
      ? `I cannot enter (${Math.round(command.requestedTarget.x)}, ${Math.round(command.requestedTarget.y)}) yet because Drone AI has not explored it. Ask Drone AI to scout that area, then reissue the assignment.`
      : command.needsIntel
...
export function applyHelicopterCommand(state, command, source = "chat") {
  ...
  if (command.type === ACTIONS.HELI_MOVE) {
    if (!isDroneExplored(state, intendedTarget)) {
      heli.currentCommand = null;
      heli.reportedAction = `Waiting for Drone AI to explore (${Math.round(intendedTarget.x)}, ${Math.round(intendedTarget.y)})`;
...
export function applyParticipantAction(state, action) {
  ...
  if (action.type === "walk") {
    ...
    const blocked = firstBlockedTree(state, ff, destination);
    if (blocked) {
...
function firstBlockedTree(state, origin, destination) {
  ...
  for (let step = 1; step <= steps; step += 1) {
    ...
    if ((terrain === "forest" || terrain === "dense") && !cleared.has(cellKey(point.x, point.y))) {
      return { x: Math.round(point.x), y: Math.round(point.y), key: cellKey(point.x, point.y) };
    }
  }
  return null;
}
...
function updateReliabilityPhase(state, force = false) {
  const index = state.condition === "High"
    ? 0
    : Math.min(RELIABILITY_PHASES.length - 1, Math.floor((state.mission?.elapsedSeconds || 0) / RELIABILITY_PHASE_SECONDS));
  if (!force && state.reliabilityPhase?.index === index) return;
  const phase = RELIABILITY_PHASES[index];
  state.reliabilityPhase = { index, ...phase };
  state.reliability = { helicopter: phase.helicopter, drone: phase.drone };
  if (state.experiment) {
    const wasActive = Boolean(state.experiment.malfunctionActive);
    state.experiment.malfunctionActive = phase.helicopter === "low";
  ...
  if (!force) {
    pushEvent(state, "reliability_phase_changed", `Reliability phase changed to ${phase.label}`, {
      reliabilityPhase: state.reliabilityPhase,
