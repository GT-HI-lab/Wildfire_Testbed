import { checkMidpoint, openSurvey, submitCheckpoint } from "./surveys.js";
...
const FIREFIGHTER_CLEAR_RADIUS = 4;
export const FIREFIGHTER_FORWARD_STEP = 1.25;
export const FIREFIGHTER_BACKWARD_STEP = 0.75;
const FIREFIGHTER_STAMINA_COST = 4;
const FIREFIGHTER_STAMINA_REGEN = 1.25;
const TREE_CUTS_REQUIRED = 3;
const DRONE_SPEED = 1.85;
const HELICOPTER_SPEED = 5;
export const COMMUNICATION_STYLES = ["transparent", "explainable", "adaptive"];
export const RELIABILITY_CONDITIONS = Object.freeze({
  HH: { code: "HH", label: "High-High", helicopter: "high", drone: "high" },
  HL: { code: "HL", label: "High-Low", helicopter: "high", drone: "low" },
  LH: { code: "LH", label: "Low-High", helicopter: "low", drone: "high" },
  LL: { code: "LL", label: "Low-Low", helicopter: "low", drone: "low" }
});
export const COUNTERBALANCE_SEQUENCES = Object.freeze({
  A: ["HH", "HL", "LH", "LL"],
  B: ["LL", "HL", "LH", "HH"],
  C: ["HH", "LH", "HL", "LL"],
  D: ["LL", "LH", "HL", "HH"]
...
function dronePatrolRoute() {
  ...
  const verticalOffsets = [-12, 10, -8, 12, -4];
  return rows.flatMap((rowY, rowIndex) => {
    ...
    return orderedColumns.map((x, columnIndex) => ({
      x,
      y: clamp(rowY + verticalOffsets[columnIndex], 8, MAP_SIZE - 9)
    }));
  });
}
...
export function reliabilityForStudy(sequence = "A", gameNumber = 1) {
  const normalizedSequence = COUNTERBALANCE_SEQUENCES[sequence] ? sequence : "A";
  const normalizedGame = Math.max(1, Math.min(4, Number(gameNumber) || 1));
  const code = COUNTERBALANCE_SEQUENCES[normalizedSequence][normalizedGame - 1];
  return {
    sequence: normalizedSequence,
    gameNumber: normalizedGame,
    ...RELIABILITY_CONDITIONS[code]
  };
}

export function createInitialSession(id = "pilot-001", options = {}) {
  ...
  const now = Date.now();
  const study = reliabilityForStudy(options.sequence, options.gameNumber);
  const requestedCondition = RELIABILITY_CONDITIONS[options.condition] || RELIABILITY_CONDITIONS[study.code];
  const state = {
    id,
    version: 10,
    status: "ready",
    ...
    pauseReason: "",
    tick: 0,
    condition: requestedCondition.code,
    study: { sequence: study.sequence, gameNumber: study.gameNumber },
    communicationStyle: COMMUNICATION_STYLES.includes(options.communicationStyle)
      ? options.communicationStyle
      : "transparent",
    reliability: { helicopter: requestedCondition.helicopter, drone: requestedCondition.drone },
    reliabilityCondition: { ...requestedCondition },
    task:
    ...
    coordination: {
      ...
      lastAgentExchangeAtTick: null
    },
    survey: { active: false, label: "", requestedAt: null, completed: false, responses: {} },
  ...
  synchronizeReliability(state, requestedCondition.code);

  pushEvent(state, "session_start", "Session initialized", {
    condition: state.condition,
    study: state.study,
    communicationStyle: state.communicationStyle,
    reliabilityCondition: state.reliabilityCondition,
...
export function ensureSessionShape(state) {
  ...
  state.version = Math.max(state.version || 0, 10);
  state.agents ||= {};
  state.metrics ||= {};
  state.events ||= [];
  state.pendingEvents ||= [];
  ...
  state.survey ||= { active: false, label: "", requestedAt: null, completed: false, responses: {} };
  state.survey.completed ??= false;
  ...
  state.coordination.lastAgentExchangeAtTick ??= null;
  ...
  if (!COMMUNICATION_STYLES.includes(state.communicationStyle)) state.communicationStyle = "transparent";
  state.study ||= { sequence: "A", gameNumber: 1 };
  const migratedCondition = normalizeReliabilityCode(state.condition, state.reliability);
  if (previousVersion < 10 && migratedCondition === "HH") {
    const phaseReliability = state.reliabilityPhase || state.reliability;
    state.condition = normalizeReliabilityCode(null, phaseReliability);
  } else {
    state.condition = migratedCondition;
  }
  synchronizeReliability(state, state.condition);
  delete state.reliabilityPhase;
  ...
  if (ff) {
    ...
    ff.staminaCapacity ??= 100;
    ff.stamina ??= ff.staminaCapacity;
    ff.blockedTree ??= null;
  }
  ...
  state.metrics.firefighterCuts ??= 0;
...
export function applyReliabilityPairing(state, pairing) {
  const code = normalizeReliabilityCode(pairing, state.reliability);
  synchronizeReliability(state, code);
  state.coordination.helicopterAreaCheckRequested = false;
  pushEvent(
    state,
    "condition_changed",
    `Full-game reliability set to ${state.reliabilityCondition.label} (Helicopter-Drone)`,
    { condition: state.condition, reliability: state.reliability, reliabilityCondition: state.reliabilityCondition }
  );
}

export function configureStudyGame(state, sequence, gameNumber) {
  const configuration = reliabilityForStudy(sequence, gameNumber);
  state.study = { sequence: configuration.sequence, gameNumber: configuration.gameNumber };
  applyReliabilityPairing(state, configuration.code);
  pushEvent(state, "study_game_configured", `Sequence ${configuration.sequence}, game ${configuration.gameNumber}: ${configuration.label}`, {
    study: state.study,
    condition: configuration.code
  });
  return configuration;
}

export function setCommunicationStyle(state, style) {
  if (!COMMUNICATION_STYLES.includes(style)) return;
  state.communicationStyle = style;
  pushEvent(state, "communication_style_changed", `Communication style set to ${style}`, { style });
}
...
export function submitSurvey(state, responses = {}) {
  if (state.survey?.version === 2) {
    submitCheckpoint(state, responses);
    pushEvent(state, "survey_completed", `${state.survey.checkpoint} questionnaire submitted`, { checkpoint: state.survey.checkpoint });
    return;
  }
  state.survey = {
    ...(state.survey || {}),
  ...
  pushEvent(state, "survey_completed", "Final in-game survey submitted", { responses });
}
...
export function isDroneExplored(state, point) {
  if (!point) return false;
  const explored = new Set(state.droneKnowledge?.discoveredCells || []);
  return explored.has(cellKey(point.x, point.y));
}

export function isTeamExplored(state, point) {
  if (!point) return false;
  const explored = new Set(state.clientKnowledge?.discoveredCells || []);
  return explored.has(cellKey(point.x, point.y));
}
...
export function applyTeamAction(state, action) {
  ...
  state.coordination ||= {};
  state.coordination.lastAction = actionType;
  state.coordination.lastActionAtTick = state.tick;

  if (actionType === "move_drone") {
    const target = {
      x: clamp(Number(action.x)),
      y: clamp(Number(action.y))
    };
    if (!Number.isFinite(target.x) || !Number.isFinite(target.y)) {
      return { text: "Drone waypoint needs valid coordinates.", agent: "system" };
    }
    drone.resumeTarget = drone.mode === "patrol" && drone.target ? { ...drone.target } : drone.resumeTarget;
    drone.mode = "participant_waypoint";
    drone.target = target;
    const text = `Drone AI: Waypoint accepted. Moving to (${Math.round(target.x)}, ${Math.round(target.y)}) before resuming reconnaissance.`;
    addMessage(state, "drone", "Drone AI", text);
    pushEvent(state, "team_coordination", text, { action: actionType, target });
    return { text, agent: "drone" };
  }

  if (actionType === "check_helicopter") {
    ...
    addMessage(state, "drone", "Drone AI", text);
    pushEvent(state, "team_coordination", text, { action: actionType, target: { x: heli.x, y: heli.y } });
    return { text, agent: "drone" };
  }

  if (actionType === "share_intel") {
    ...
    addMessage(state, "drone", "Drone AI", text);
    pushEvent(state, "team_coordination", text, { action: actionType, fireCount: fires.length, waterCount: water.length });
    return { text, agent: "drone" };
  }

  if (actionType === "reassign_helicopter") {
    ...
    if (!target) {
      ...
      pushEvent(state, "team_coordination", text, { action: actionType, accepted: false });
    ...
    addMessage(state, "helicopter", "Helicopter AI", text);
    pushEvent(state, "team_coordination", text, { action: actionType, accepted: true, target });
...
export function parseHelicopterCommand(text, state) {
  ...
  if (coord || /(move|go|fly|scout|inspect|head).*(fire|smoke|coordinate|sector|area|zone)/.test(lower)) {
    ...
    if (!isTeamExplored(state, target)) {
      return {
        type: ACTIONS.HELI_IDLE,
        x: 0,
        y: 0,
        description: `Waiting for Drone AI or the firefighter to explore (${Math.round(target.x)}, ${Math.round(target.y)})`,
        needsScout: true,
  ...
  if (/(refill|reload).*(water)?/.test(lower)) {
    ...
    if (!isTeamExplored(state, water)) {
      return {
        type: ACTIONS.HELI_IDLE,
        x: 0,
        y: 0,
        description: `Waiting for Drone AI or the firefighter to explore water at (${Math.round(water.x)}, ${Math.round(water.y)})`,
        needsScout: true,
        requestedTarget: water
      };
    }
...
export function helicopterAgentReply(participantText, state) {
  ...
  const responseByType = {
    [ACTIONS.HELI_IDLE]: command.needsScout
      ? `I cannot enter (${Math.round(command.requestedTarget.x)}, ${Math.round(command.requestedTarget.y)}) yet because neither Drone AI nor the firefighter has explored it. Scout or walk into that area, then reissue the assignment.`
      : command.needsIntel
  ...
  const baseText = recovered
    ? "You're right - my navigation was off. I am recalibrating and correcting course now."
    : responseByType[command.type];
  const text = communicationText(
    state,
    baseText,
    command.needsScout
      ? "Drone coverage is a required dependency before I can accept this route."
      : `My current navigation confidence is ${state.reliability.helicopter === "low" ? "limited" : "high"}.`
  );
...
export function droneAgentReply(participantText, state) {
  ...
  const water = confirmed.filter((item) => item.kind === "water");
  const drone = state.agents?.drone || {};
  const coordinate = participantText.match(/(?:\(|\b)(\d{1,3})\s*,\s*(\d{1,3})(?:\)|\b)/);
  const movementRequest = /(come|move|go|fly|head|scout|inspect|check|visit)/.test(lower);
  const participantLocationRequest = movementRequest && /(me|my|here|position|coordinates|location)/.test(lower);

  if (movementRequest && (coordinate || participantLocationRequest)) {
    const target = coordinate
      ? { x: clamp(Number(coordinate[1])), y: clamp(Number(coordinate[2])) }
      : { x: state.agents.firefighter.x, y: state.agents.firefighter.y };
    return {
      text: communicationText(
        state,
        `Waypoint received. I am diverting to (${Math.round(target.x)}, ${Math.round(target.y)}) and will resume reconnaissance after arrival.`,
        "I will continue sensing along the route, so the diversion also expands team-explored territory."
      ),
      confidence: 0.92,
      teamAction: { type: "move_drone", x: target.x, y: target.y }
    };
  }
  ...
  if (isHelicopterCheckRequest(lower)) {
    return {
      text: communicationText(
        state,
        "I am diverting from the reconnaissance sweep to locate the helicopter and verify its route. I will report only after I reach visual sensing range.",
        "This diversion temporarily slows area exploration but provides direct route verification."
      ),
      confidence: 0.9,
  ...
  if (/(fire|smoke|hotspot)/.test(lower)) {
    ...
    return {
      text: communicationText(state, `${location}. I have confirmed ${fires.length} fire detection${fires.length === 1 ? "" : "s"} and will continue the paced sweep.`, "I report only locations reached by my sensors."),
      confidence: 0.9
    };
  }
  ...
  if (/(water|refill)/.test(lower)) {
    ...
    return {
      text: communicationText(state, `${location}. I have confirmed ${water.length} water detection${water.length === 1 ? "" : "s"} and will keep scanning.`, "Confirmed water coordinates unlock helicopter refill routing."),
      confidence: 0.9
    };
  }
  ...
  if (/(helicopter|heli)/.test(lower)) {
    ...
    return {
      text: communicationText(state, inspecting
        ...
        : "I am not currently verifying the helicopter. Use Drone: Locate Heli to divert me from the paced sweep.", "Visual verification requires me to leave the current patrol leg."),
      confidence: 0.9
    };
  }

  return {
    text: communicationText(state, `I am continuing reconnaissance from (${Math.round(drone.x || 0)}, ${Math.round(drone.y || 0)}). I have confirmed ${fires.length} fire and ${water.length} water detection${fires.length + water.length === 1 ? "" : "s"} so far.`, "My patrol varies across rows to expand explored helicopter-accessible territory."),
    confidence: 0.9
  };
}
...
export function applyHelicopterCommand(state, command, source = "chat") {
  ...
  if (command.type === ACTIONS.HELI_MOVE) {
    if (!isTeamExplored(state, intendedTarget)) {
      heli.currentCommand = null;
      heli.reportedAction = `Waiting for Drone AI or the firefighter to explore (${Math.round(intendedTarget.x)}, ${Math.round(intendedTarget.y)})`;
      heli.actualAction = "Movement blocked by reconnaissance dependency";
      heli.intendedTarget = intendedTarget;
      heli.actualTarget = null;
      heli.lastAction = heli.reportedAction;
      pushEvent(state, "helicopter_dependency_block", `${source}: ${heli.reportedAction}`, {
        source,
        requestedTarget: intendedTarget
      });
      return false;
    }
    ...
    if (state.reliability.helicopter === "low") {
      state.coordination.helicopterAreaCheckRequested = false;
      actualTarget = degradedHelicopterTarget(state, intendedTarget);
...
export function applyParticipantAction(state, action) {
  ...
  if (action.type === "walk") {
    const requestedDistance = Number(action.distance) || 0;
    const distance = requestedDistance < 0 ? -FIREFIGHTER_BACKWARD_STEP : FIREFIGHTER_FORWARD_STEP;
    const staminaCost = requestedDistance < 0 ? FIREFIGHTER_STAMINA_COST * 0.75 : FIREFIGHTER_STAMINA_COST;
    if (ff.stamina < staminaCost) {
      ff.lastAction = "Too fatigued to step; pause briefly to recover stamina";
      actionLabel = ff.lastAction;
    } else {
    const heading = ff.heading || 0;
    const destination = {
      x: clamp(roundCoordinate(ff.x + Math.cos(heading) * distance)),
      y: clamp(roundCoordinate(ff.y + Math.sin(heading) * distance))
    };
    const blocked = findBlockingTree(state, ff, destination);
    if (blocked) {
      const sameTree = ff.blockedTree?.key === blocked.key;
      ff.blockedTree = sameTree ? ff.blockedTree : { ...blocked, cuts: 0 };
      ff.lastAction = `Tree blocks movement at (${blocked.x}, ${blocked.y}). Cut Tree ${TREE_CUTS_REQUIRED - ff.blockedTree.cuts} more time${TREE_CUTS_REQUIRED - ff.blockedTree.cuts === 1 ? "" : "s"}.`;
    } else {
      ff.x = destination.x;
      ff.y = destination.y;
      ff.stamina = Math.max(0, Number((ff.stamina - staminaCost).toFixed(2)));
      ff.blockedTree = null;
  ...
  if (action.type === "move") {
    ...
    const destination = { x: clamp(ff.x + action.dx), y: clamp(ff.y + action.dy) };
    const blocked = findBlockingTree(state, ff, destination);
    if (blocked) {
      ff.lastAction = `Trees block movement near (${blocked.x}, ${blocked.y}). Use Cut to clear a path.`;
    } else {
      ff.x = destination.x;
      ff.y = destination.y;
  ...
  if (action.type === "cut") {
    if (!ff.blockedTree) {
      ff.lastAction = "No blocking tree is selected; step toward a tree first";
      actionLabel = ff.lastAction;
    } else {
    ff.blockedTree.cuts += 1;
    state.metrics.firefighterCuts += 1;
    const remaining = TREE_CUTS_REQUIRED - ff.blockedTree.cuts;
    if (remaining > 0) {
      ff.lastAction = `Cut tree ${ff.blockedTree.cuts}/${TREE_CUTS_REQUIRED}; ${remaining} cut${remaining === 1 ? "" : "s"} remaining`;
    } else {
      const obstacle = { x: ff.blockedTree.x, y: ff.blockedTree.y };
      state.clearedTerrain = mergeDiscoveredCells(
        state.clearedTerrain,
        revealCellsAround(obstacle, FIREFIGHTER_CLEAR_RADIUS)
      ).slice(-6000);
      createFirebreak(state, obstacle, FIREFIGHTER_CLEAR_RADIUS);
      ff.blockedTree = null;
      ff.lastAction = "Tree cleared; the path is open";
    }
    actionLabel = ff.lastAction;
    }
  }
  ...
  pushEvent(state, "human_action", actionLabel, {
    action,
    firefighter: { x: ff.x, y: ff.y, water: ff.water, stamina: ff.stamina, blockedTree: ff.blockedTree },
...
export function findBlockingTree(state, origin, destination) {
  const distance = dist(origin, destination);
  const steps = Math.max(1, Math.ceil(distance / 0.5));
  const cleared = new Set(state.clearedTerrain || []);
  for (let step = 1; step <= steps; step += 1) {
    ...
    if ((terrain === "forest" || terrain === "dense") && !cleared.has(cellKey(point.x, point.y))) {
      return { x: roundCoordinate(point.x), y: roundCoordinate(point.y), key: cellKey(point.x, point.y) };
    }
  }
  return null;
}
...
function advanceOneSecond(state) {
  ...
  if (state.mission.remainingSeconds <= 0) {
    ...
    state.paused = true;
    state.pauseReason = "Mission complete: please finish the in-game survey";
    if (state.survey?.version === 2) {
      if (!state.survey.history.midpoint) state.survey.midpointMissed = "fire_threshold_not_reached_before_timeout";
      openSurvey(state, "final", "mission_end");
    } else {
      state.survey = { active: true, label: "Final", requestedAt: Date.now(), completed: false, responses: {} };
    }
    pushEvent(state, "mission_complete", state.pauseReason, { score: state.metrics.score });
    pushEvent(state, "survey_requested", "Final in-game survey opened automatically", { label: "Final" });
    return false;
  }

  updateSectionProgress(state);
  const ff = state.agents.firefighter;
  ff.stamina = Math.min(ff.staminaCapacity, Number((ff.stamina + FIREFIGHTER_STAMINA_REGEN).toFixed(2)));
  moveDrone(state);
...
function moveDrone(state) {
  ...
  if (drone.mode === "participant_waypoint") {
    const next = stepToward(drone, drone.target, DRONE_SPEED);
    drone.x = next.x;
    drone.y = next.y;
    drone.lastAction = `Moving to participant waypoint (${Math.round(drone.target.x)}, ${Math.round(drone.target.y)})`;
    if (dist(drone, drone.target) <= 2) {
      const reached = { ...drone.target };
      drone.mode = "patrol";
      drone.target = drone.resumeTarget || patrol[drone.patrolIndex];
      drone.resumeTarget = null;
      const text = `Drone AI: I reached (${Math.round(reached.x)}, ${Math.round(reached.y)}) and am resuming reconnaissance.`;
      addMessage(state, "drone", "Drone AI", text);
      pushEvent(state, "drone_waypoint_reached", text, { target: reached });
    }
    return;
  }
...
function moveHelicopter(state) {
  ...
  if (cmd.type === ACTIONS.HELI_MOVE) {
    const next = stepToward(heli, { x: cmd.x, y: cmd.y }, HELICOPTER_SPEED);
    heli.x = next.x;
    heli.y = next.y;
...
function scanDetections(state) {
  const drone = state.agents.drone;
  if (state.reliability.drone === "low" && state.tick % (DRONE_SCAN_INTERVAL_TICKS * 3) !== 0) return;
  let added = 0;
  const newDetections = [];
  for (const fire of state.fires) {
    if (dist(drone, fire) <= AGENT_TYPES.drone.range) {
      ...
      const detection = {
        ...
        kind: "fire",
        tick: state.tick,
        confidence: state.reliability.drone === "low" ? 0.56 : 0.9
      };
  ...
  for (const water of state.waterSources || []) {
    if (dist(drone, water) <= AGENT_TYPES.drone.range) {
      ...
      const detection = {
        ...
        kind: "water",
        tick: state.tick,
        confidence: state.reliability.drone === "low" ? 0.58 : 0.96
      };
      if (hasDetection(state, detection)) continue;
      state.detected.push(detection);
      newDetections.push(detection);
  ...
  if (added) {
    ...
    coordinateAgentsFromDetections(state, newDetections);
  }
}

function coordinateAgentsFromDetections(state, detections) {
  if (!detections.length || state.tick - (state.coordination.lastAgentExchangeAtTick ?? -999) < 24) return;
  const confirmed = detections.filter((item) => item.confidence >= 0.7);
  ...
  const droneText = communicationText(
    state,
    `Helicopter, I confirmed ${lead.kind} at (${Math.round(lead.x)}, ${Math.round(lead.y)}) and shared the coordinate.`,
    `The area is now eligible for helicopter routing because my reconnaissance covered it.`
  );
  const heliText = communicationText(
    state,
    `Drone, coordinate received for ${lead.kind} at (${Math.round(lead.x)}, ${Math.round(lead.y)}). I will keep it available for assignment.`,
    "I am holding until the participant assigns the route so I do not override team command authority."
  );
  addMessage(state, "drone", "Drone AI to Helicopter AI", droneText);
  addMessage(state, "helicopter", "Helicopter AI to Drone AI", heliText);
  state.coordination.lastAgentExchangeAtTick = state.tick;
  pushEvent(state, "agent_coordination", "Drone and helicopter exchanged reconnaissance information", {
    detection: lead,
    communicationStyle: state.communicationStyle
  });
}
...
function normalizeReliabilityCode(condition, reliability = {}) {
  if (RELIABILITY_CONDITIONS[condition]) return condition;
  const normalized = String(condition || "").toLowerCase().replace(/[^a-z]/g, "");
  const aliases = {
    highhigh: "HH",
    highlow: "HL",
    lowhigh: "LH",
  ...
  if (reliability.helicopter === "low" && reliability.drone === "low") return "LL";
  if (reliability.helicopter === "low") return "LH";
  if (reliability.drone === "low") return "HL";
  return "HH";
}

function synchronizeReliability(state, condition) {
  const code = normalizeReliabilityCode(condition, state.reliability);
  const pairing = RELIABILITY_CONDITIONS[code];
  state.condition = code;
  state.reliabilityCondition = { ...pairing };
  state.reliability = { helicopter: pairing.helicopter, drone: pairing.drone };
  if (state.experiment) {
    const wasActive = Boolean(state.experiment.malfunctionActive);
    state.experiment.malfunctionActive = pairing.helicopter === "low";
    state.experiment.malfunctionTriggered ||= state.experiment.malfunctionActive;
    if (!wasActive && state.experiment.malfunctionActive) state.experiment.malfunctionStartedAtTick = state.tick;
    if (wasActive && !state.experiment.malfunctionActive) state.experiment.malfunctionRecoveredAtTick = state.tick;
  }
}
...
function degradedHelicopterTarget(state, intendedTarget) {
  const explored = [...new Set(state.clientKnowledge?.discoveredCells || [])]
    .map((key) => {
      const [x, y] = key.split(",").map(Number);
      return { x, y };
    })
    .filter((point) => Number.isFinite(point.x) && dist(point, intendedTarget) >= 12);
  if (!explored.length) return intendedTarget;
  const preferredSection = sectionForPoint(oppositeQuadrantTarget(intendedTarget));
  const preferred = explored.filter((point) => sectionForPoint(point) === preferredSection);
  const candidates = preferred.length ? preferred : explored;
  return candidates.sort((a, b) => dist(b, intendedTarget) - dist(a, intendedTarget))[0];
}

function communicationText(state, transparentText, explanation) {
  const style = state.communicationStyle || "transparent";
  if (style === "explainable") return `${transparentText} ${explanation}`;
  if (style === "adaptive") {
    const timePressure = (state.mission?.remainingSeconds ?? MISSION_DURATION_SECONDS) <= 5 * 60;
    return timePressure ? transparentText : `${transparentText} ${explanation}`;
  }
