import detectionEvents from "../netlify/functions/detection-events.mjs";
...
const publicRoot = process.env.WILDFIRE_SERVE_DIST === "true" ? join(root, "dist") : root;
...
const functions = {
  "/.netlify/functions/detection-events": detectionEvents,
...
server.listen(port, "127.0.0.1", () => {
  ...
  console.log(`CREW Wildfire local study: http://127.0.0.1:${listeningPort}/client/?preview=1&wave=1`);
  console.log(`Administrator dashboard: http://127.0.0.1:${listeningPort}/server/ (use STUDY_ADMIN_KEY; default: local-admin)`);
});
...
async function serveStatic(pathname, outgoing) {
  ...
  if (!["client", "server", "shared"].includes(parts[0]) || parts.some((part) => part.startsWith(".") || part === "env.js")) return notFound(outgoing);
  let filename = resolve(publicRoot, relative);
  if (!filename.startsWith(`${publicRoot}/`)) return notFound(outgoing);
  try {
