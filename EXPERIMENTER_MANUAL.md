...
The dashboard is optional. Closing it does not stop an active participant game. The participant browser runs the simulation and must stay open during play; Netlify stores checkpoints, and the clock can catch up from its stored deadline after re-entry.
...
7. Verify the configured baseline and midpoint/final questionnaires using `QUESTIONNAIRES.md`, including the required SQL upgrade.
...
At `/server/`, select **Pause New Sessions**. This blocks enrollment requests, including re-entry through enrollment, while allowing already connected sessions to continue. Select **Resume** when the problem is resolved.
...
Keep question IDs stable after data collection begins. This edition uses required numeric rating choices with explicit missing-value handling for MDMT. See `QUESTIONNAIRES.md` for the current protocol and data fields.
