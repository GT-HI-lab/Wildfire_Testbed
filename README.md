...
Start with [ONLINE_SETUP_GUIDE.md](ONLINE_SETUP_GUIDE.md) for the complete local-to-online handoff. See [VERIFICATION.md](VERIFICATION.md) for tested behavior and remaining limitations. The supplied questionnaires and midpoint checkpoint are implemented; see [QUESTIONNAIRES.md](QUESTIONNAIRES.md) for the protocol and required database upgrade. The visual detection secondary task is implemented locally; see [SECONDARY_TASK.md](SECONDARY_TASK.md) for timing, data and deployment details.
...
4. Baseline is collected before game 1; MDMT and workload questionnaires open at midpoint and mission end in every game.
