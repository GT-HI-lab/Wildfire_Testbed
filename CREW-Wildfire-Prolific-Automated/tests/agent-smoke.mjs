...
state.droneKnowledge.discoveredCells.push("180,60");
state.clientKnowledge.discoveredCells.push("180,60");
const configResponse = await configHandler();
const runtimeConfig = JSON.parse(configResponse.body);
assert(!("QUALTRICS_SURVEY_URL" in runtimeConfig), "runtime config no longer exposes an external survey URL");
assert(!runtimeConfig.AI_ENABLED && runtimeConfig.AI_STATUS_CODE === "missing_configuration", "missing Function key is diagnosed safely");
...
const waypointResponse = await handler({
  httpMethod: "POST",
  body: JSON.stringify({
    agent: "drone",
    message: "Come to my coordinates",
    state
  })
});
const waypoint = JSON.parse(waypointResponse.body);
assert(waypoint.teamAction?.type === "move_drone", "drone waypoint request survives the serverless API boundary");
assert(waypoint.teamAction.x === state.agents.firefighter.x, "drone waypoint uses the participant position");
...
const healthyConfigResponse = await configHandler();
const healthyConfig = JSON.parse(healthyConfigResponse.body);
assert(healthyConfig.AI_ENABLED && healthyConfig.AI_STATUS_CODE === "ready", "config verifies provider access");
assert(!("AI_PROVIDER" in healthyConfig) && !("AI_MODEL" in healthyConfig), "public config hides provider and model details");
...
assert(shortReply.diagnosticCode === "short_response", "short reply has a safe diagnostic code");
...
const quotaResponse = await handler({
  httpMethod: "POST",
  body: JSON.stringify({ agent: "drone", message: "Report status", state })
});
const quotaReply = JSON.parse(quotaResponse.body);
assert(quotaReply.diagnosticCode === "quota_exceeded", "provider quota errors are identified safely");
assert(!quotaReply.diagnostic.includes("quota details"), "raw provider errors are not exposed to the participant");
