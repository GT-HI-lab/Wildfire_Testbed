import { validateResponses } from "../../shared/survey-config.js";
...
export default async function sessionState(request) {
  ...
  if (request.method === "PUT" || request.method === "POST") {
    ...
    for (let attempt = 0; attempt < 4; attempt += 1) {
      ...
      try { reconcileSurvey(state, current); }
      catch (error) { return Response.json({ error: error.message }, responseOptions(400)); }
      ...
      if (result.modified) {
        if (changedEvents.length || changedMessages.length || state.survey?.completed || Object.keys(state.survey?.history || {}).length) {
...
function reconcileSurvey(state, current) {
  if (current?.survey?.version !== 2) return;
  if (state.survey?.version !== 2) throw new Error("Questionnaire state is required.");
  const history = { ...(state.survey.history || {}), ...current.survey.history };
  for (const [checkpoint, entry] of Object.entries(history)) {
    if (!["baseline", "midpoint", "final"].includes(checkpoint) || entry.checkpoint !== checkpoint) throw new Error("Invalid questionnaire checkpoint.");
    if (checkpoint === "baseline" && state.study.gameNumber !== 1) throw new Error("Baseline belongs to game 1 only.");
    validateResponses(checkpoint, entry.responses);
    if (!Number.isFinite(entry.completedAt)) throw new Error("Questionnaire timestamp is required.");
  }
  if (history.final && !history.midpoint) throw new Error("Mid-game questionnaire must be submitted first.");
  if ((history.midpoint || history.final) && state.study.gameNumber === 1 && !history.baseline) throw new Error("Baseline questionnaire must be submitted first.");
  state.survey.history = history;
  // Stale tabs cannot replace already submitted questionnaires or reopen them.
  if (state.survey.active && history[state.survey.checkpoint]) {
    state.survey = { ...current.survey, history };
    state.paused = current.paused;
    state.status = current.status;
    state.pauseReason = current.pauseReason;
  }
  if (current.survey.active && !history[current.survey.checkpoint]) {
    state.survey = { ...current.survey, history };
    state.paused = true;
    state.mission = { ...current.mission };
    state.pauseReason = current.pauseReason;
  }
  state.survey.completed = Boolean(history.final);
}
