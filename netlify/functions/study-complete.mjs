import { surveyCompleteForGame } from "../../shared/surveys.js";
...
export default async function studyComplete(request) {
  ...
  if (!state?.mission?.completed || !surveyCompleteForGame(state)) {
    ...
    const next = {
      ...
      waves: {
        ...
        [String(metadata.wave)]: {
          ...
          surveyResponseCount: state.survey?.version === 2 ? Object.values(state.survey.history).reduce((sum, entry) => sum + Object.keys(entry.responses).length, 0) : Object.keys(state.survey?.responses || {}).length
        }
      },
      updatedAt: now
    };
