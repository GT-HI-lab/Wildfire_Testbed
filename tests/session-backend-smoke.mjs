...
try {
  ...
  const serverState = { ...saved, tick: 13, mission: { elapsedSeconds: 13, remainingSeconds: 1787 } };
  await sessionState(new Request(url, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
  ...
  const staleClientState = {
    ...saved,
    tick: 12,
    mission: { elapsedSeconds: 12, remainingSeconds: 1788 },
    messages: [{ id: "chat-1", at: 1, text: "Concurrent participant message" }]
  };
  const concurrentWrite = await sessionState(new Request(url, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ state: staleClientState })
  }));
  const concurrentPayload = await concurrentWrite.json();
  assert(concurrentWrite.status === 200, `concurrent write merges successfully: ${JSON.stringify(concurrentPayload)}`);
  const concurrent = concurrentPayload.state;
  assert(concurrent.tick === 13 && concurrent.mission.elapsedSeconds === 13, "stale client write cannot rewind mission time");
  assert(concurrent.messages.some((message) => message.id === "chat-1"), "concurrent participant message is preserved");
